...
```{code-cell} ipython3
#la masse est un objet et non un flottant
```
...
```{code-cell} ipython3
df['Mass (lb) orig'] = pd.to_numeric(df['Mass (lb) orig'], errors = 'coerce')  
```
...
```{code-cell} ipython3
df['Mass2'] = df['Mass (lb)'].copy()
df['Mass2'] = pd.to_numeric(df['Mass2'].str.replace(">", "").str.replace("<", ""))
df['Mass2'].dtype
```
...
```{code-cell} ipython3
mask = df['Artificial object'].str.contains('Memorial', case=False, na=False)
df[mask]
#le Luxembourg a mis ce mémorial 
```
...
```{code-cell} ipython3
objects_list = df['Artificial object'].tolist()
print(objects_list)
```
