...
```{code-cell} ipython3
# J'ai écris une version avancée ci-dessous qui semble fonctionneer, je l'ai cependant mis avec des'#' car j'ai utilisé pour la suite du
# tp la "méthode pas-à-pas" qui était plus sure.
# VERSION AVANCÉE (utilisation d'une boucle for) 
# Il faut réorganiser le dictionnaire pour que ses données puissent être inteerprétées par la commande pd.DataFrame. 
# Il faut rajouter à la liste des données, le pays. 
...
```{code-cell} ipython3
# Nombre de ligne (entrée/ mesures)
fr.shape[0] 
```
...
for pays in by_country.keys() :
    courant = pd.DataFrame(by_country[pays])
    courant['country'] = pays
    list_to_concat.append(courant)

global_df = pd.concat(list_to_concat)
```
...
global_df.loc[0]
#en effet dans chaque 'sous data frame' correspondant à chaque pays, les premier cas sont indéxé par 0, il y'a donc 
#un bon noùbre de ligne avec 0 pour index 
```
...
```{code-cell} ipython3
# Il porte a confusion on sait pas distinguer le deuxieme et le troisieme nombre. 
# Ça n'est donc probablement pas pratique pour calculer la durée entre deux évènement car cela demande d'abord de déduire le sens 
# de la date
# De plus, le format présenté dans l'encadré en en-tête semble favorable car il permet d'acceder séparément au jour, au moins ou à l'année. 
# Pour améliorer cela on pourrait réecrire la colonnes 'dates' en imposant une convention de date.
```
...
```{code-cell} ipython3
#22 ne pouvait être que le jour, il convient d'adopter le format suivant : 

pd.to_datetime('2020-1-22', format='%Y-%m-%d')
```
...
```{code-cell} ipython3
global_df['date_ord'] = pd.to_datetime(global_df['date'], format='%Y-%m-%d')
```
...
```{code-cell} ipython3
global_df['date'] = global_df['date_ord']     #on remplace 
global_df.drop(columns='date_ord', inplace=True)    #on supprime la temporaire 
```
...
```{code-cell} ipython3
# Il serait pertinent d'utiliser un multiIndex et d'indexer la data frame par le tuple ('country', 'date')
```
...
```{code-cell} ipython3
global_df_new = global_df.pivot_table( values = ['confirmed', 'deaths',	'recovered'], index = ['country', 'date']) 
```
...
```{code-cell} ipython3
global_df_new.loc[('France', '2021-01-01'), ['confirmed', 'deaths', 'recovered']]
```
...
```{code-cell} ipython3
#je ne suis pas sur d'avoir comprris la question : j'ai extrait la data frame qui contient toute les données relative à la france grâce à un groupeby (ce qu'on a vu en cours)
df_group_fr = global_df_new.groupby(by = 'country')
france_data = df_group_fr.get_group('France')
france_data.head()
```
...
```{code-cell} ipython3
#j'applique donc la même méthode 
ita_fra_data = pd.concat([
    df_group_fr.get_group('France'), 
    df_group_fr.get_group('Italy')
])
...
```{code-cell} ipython3
france_daily = france_data.groupby(by = 'date').sum().sort_index()  #le .sum important ici car si plusieurs mesures ont la même dtes on les aditionne pour en faire qu'une seule valeur sur le graphe 
france_daily.plot()
```
...
```{code-cell} ipython3
france_daily.plot( y = ['confirmed','deaths'])
```
...
```{code-cell} ipython3
grouped = global_df_new.groupby('country')

df3 = pd.concat([
    grouped.get_group('France'),
    grouped.get_group('Italy'),
    grouped.get_group('Germany')])
...
```{code-cell} ipython3
df3.plot(rot=45)
#le paramètre ajouté permet de realiser une rotatoin de 45° des dates en légendes ce qui est plus lisible
#le problème est que 'date' n'étant pas l'unique index on observe la succession des différents pays sur le graphe. 
# cela étant peu lisible, il convient de le modifier
```
...
```{code-cell} ipython3
df3.shape
#effectivement le nombre de colonnes semble être le tripe de 1 pays 
df3.head(4)
```
...
df6.plot(rot = 45)
#les dates en abcisses apparaissent bien comme attendues.
# on obtient 6 courbes différentes (de couleurs différentes entre elles), qui rend le graphe lisible
```
...
```{code-cell} ipython3
def extract(country : list, measures : list): 
    bycountry = global_df_new.groupby('country')
    df_fir = bycountry.get_group(country[0])
    for k in range(1, len(country)) : 
        df_fir = pd.concat([df_fir, bycountry.get_group(country[k])])
    df_fir = df_fir[measures]
    df_sec = df_fir.unstack(0)
    return df_sec
```

```{code-cell} ipython3
extract(['France', 'Italy', 'Germany'] ,['confirmed', 'deaths'])
```

```{code-cell} ipython3
#je n'ai pas compris comment il était attendu d'ajouter la date à la fonction
```
